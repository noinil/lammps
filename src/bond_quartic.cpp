...
void BondQuartic::compute(int eflag, int vflag)
{
  ...
  double etan = 0;
  double etan_total = 0;
  std::ofstream forces_file("n_force_bd_qua.dat");
  std::ofstream energy_file("p_energy_bd_qua.dat");
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    //                       |_|                        //
    // ------------------ tc_test --------------------- */
    etan = k[type]*r2*ra*rb + u0[type];
    if (rsq < TWO_1_3) etan += 4.0*sr6*(sr6-1.0) + 1.0;
    etan_total += etan;
    ...
    if (rsq < cutsq[itype][jtype]) {
      ...
      if (newton_bond || i1 < nlocal) {
        ...
        //                       |_|                        //
        // ------------------ tc_test --------------------- */
        ftan[i1][0] += delx*fpair;
        ftan[i1][1] += dely*fpair;
        ftan[i1][2] += delz*fpair;
      }
      if (newton_bond || i2 < nlocal) {
        f[i2][0] -= delx*fpair;
        f[i2][1] -= dely*fpair;
        ...
        //                       |_|                        //
        // ------------------ tc_test --------------------- */
        ftan[i2][0] -= delx*fpair;
        ftan[i2][1] -= dely*fpair;
        ftan[i2][2] -= delz*fpair;
      }
  ...
  //                       |_|                        //
  // ------------------ tc_test --------------------- */
  energy_file << "Total bond quadratic energy: " << etan_total << std::endl;
  energy_file << " ================================================== "
              << std::endl;
